...
function CodeView() {
  ...
  // Switch to preview tab when user click deploy, export or preview
  useEffect(() => {
    (action?.actionType == "deploy" || action?.actionType == "export" || action?.actionType == "preview") &&
  ...
  // Generate AI code when user send a message
  useEffect(() => {
  ...
  const GenerateAiCode = async () => {
    ...
    // Update the file (AI response) to database
    await UpdateFiles({
      workspaceId: id,
      files: aiResp?.files,
    });
